...
router.post("/create", isAuth, async (req, res) => {
  try {
    let creates = await creaturesServices.create({
      ...req.body,
      owner: req.user._id,
    });
    await creates.save();
    let addtouser = await creaturesServices.addAd(creates._id, req.user._id);
...
router.get("/search", async (req, res) => {
  ...
  let email = await creaturesServices.search(creaturesName);

  if (email == undefined) {
    res.render("search", { error: "No such user!" });
    return;
  }

  let mapedEmail = email.map((x) => x.ads).flat();
  console.log(mapedEmail);
  // await creaturesServices.getOneUser();

  res.render("search", { email, mapedEmail });
});
