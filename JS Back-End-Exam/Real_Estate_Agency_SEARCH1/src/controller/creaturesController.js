...
router.get("/:id/details", async (req, res) => {
  try {
    ...
    let peopleBuy = creatures.donations.map((c) => c.user.name).join(", ");
    ...
    let isFull = false;
    let creaturesPieces = creatures.pieces;
    if (creaturesPieces == 0) {
      isFull = true;
    }
    ...
    res.render("creatures/details", { error: getErrorMessage(error) });
  }
});
