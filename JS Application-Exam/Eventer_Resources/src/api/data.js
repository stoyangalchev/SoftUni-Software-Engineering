...
export async function addFact(fact) {
  ...
  return await api.post(host + "/data/events", fact);
}

// edit listing by id
export async function editFactById(id, fact) {
  return await api.put(host + `/data/events/${id}`, fact);
}
...
export async function like(factId) {
  return await api.post(host + `/data/going`, factId);
}

export async function getTotalLikes(factId) {
  return await api.get(
    host + `/data/going?where=eventId%3D%22${factId}%22&distinct=_ownerId&count`
  );
}
...
export async function didUserLiked(factId, userId) {
  return await api.get(
    host +
      `/data/going?where=eventId%3D%22${factId}%22%20and%20_ownerId%3D%22${userId}%22&count`
  );
}
